...
function validateContactForm(userName, userEmail, userMessage){
	var error = false;
	
	if(userName.length <= 1){
		error = true;
	} else if(userEmail.length < 3 || userEmail.indexOf("@") === (-1) || userEmail.indexOf(".") === (-1)){
		error = true;
...
function sendContactEmail(){
	var userName = $('#user-name').val().serialize;
	var userEmail = $('#user-email').val().serialize;
	var userTwitter = $('#user-twitter').val().serialize;
	var userWhatsapp = $('#user-whatsapp').val().serialize;
	var userMessage = $('#user-message').html().serialize;
	var enqType = $('#enq-type').val().serialize;
	var gotcha = $('#gotcha').val().serialize;
	
	if(userTwitter.length < 1){
		userTwitter = "Not Provided";
	}
	
	if(userWhatsapp.length < 1){
		userWhatsapp = "Not Provided";
	}
	
	if(validateContactForm(userName, userEmail, userMessage)){
		...
		$.ajax({
			...
			error: function() {
				...
				$('#handy-form-btn').html("Error! Try Later");
			},
